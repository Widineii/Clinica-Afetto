...
public class LocalDatabaseSchemaPatch implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        try {
            ...
            ajustarTabelaPacienteCadernoObservacao();
    ...
    private void ajustarTabelaPacienteCadernoObservacao() {
        try {
            jdbcTemplate.execute(
                    """
            ...
            Integer colunaCriado = jdbcTemplate.queryForObject(
                    """
                    SELECT COUNT(*)
                    FROM INFORMATION_SCHEMA.COLUMNS
                    WHERE UPPER(TABLE_NAME) = 'PACIENTE_CADERNO_OBSERVACAO'
                      AND UPPER(COLUMN_NAME) = 'CRIADO_EM'
                    """,
                    Integer.class
            );
            if (colunaCriado != null && colunaCriado == 0) {
                jdbcTemplate.execute("ALTER TABLE paciente_caderno_observacao ADD COLUMN criado_em TIMESTAMP");
            }
            jdbcTemplate.update(
                    """
            ...
            removerRestricoesUnicasPacienteCadernoObservacao();
            adicionarColunaSeNaoExistir("evolucao_clinica", "VARCHAR(20)");
            adicionarColunaSeNaoExistir("lembrete_em", "TIMESTAMP");
        } catch (Exception ex) {
            log.warn("Nao foi possivel ajustar paciente_caderno_observacao no H2: {}", ex.getMessage());
        }
    }

    private void removerRestricoesUnicasPacienteCadernoObservacao() {
        try {
            ...
            for (String constraint : constraints) {
                ...
                log.info("Schema H2: removida constraint unica {} em paciente_caderno_observacao.", constraint);
            }
        } catch (Exception ex) {
            log.warn("Nao foi possivel remover constraints unicas de paciente_caderno_observacao: {}", ex.getMessage());
        }
        ...
        try {
            List<String> indicesUnicos = jdbcTemplate.queryForList(
                    ...
                    WHERE UPPER(TABLE_NAME) = 'PACIENTE_CADERNO_OBSERVACAO'
                      ...
                      AND UPPER(INDEX_NAME) NOT LIKE 'PRIMARY%'
                    """,
                    String.class
            );
            for (String index : indicesUnicos) {
                ...
                try {
                    ...
                    log.info("Schema H2: removido indice unico {} em paciente_caderno_observacao.", index);
                } catch (Exception ex) {
                    jdbcTemplate.execute(
                            "ALTER TABLE paciente_caderno_observacao DROP CONSTRAINT IF EXISTS \"" + index + "\""
                    );
                    log.info("Schema H2: removida constraint/index {} em paciente_caderno_observacao.", index);
                }
            }
        } catch (Exception ex) {
            log.warn("Nao foi possivel remover indices unicos de paciente_caderno_observacao: {}", ex.getMessage());
        }
    }

    private void adicionarColunaSeNaoExistir(String coluna, String tipoSql) {
        try {
            Integer existe = jdbcTemplate.queryForObject(
                    """
                    SELECT COUNT(*)
                    ...
                    """,
                    Integer.class,
                    coluna.toUpperCase(Locale.ROOT)
            );
            if (existe != null && existe == 0) {
                jdbcTemplate.execute(
                        "ALTER TABLE paciente_caderno_observacao ADD COLUMN " + coluna + " " + tipoSql
                );
            }
        } catch (Exception ex) {
            log.warn("Nao foi possivel adicionar coluna {} em paciente_caderno_observacao: {}", coluna, ex.getMessage());
        }
    }
}
